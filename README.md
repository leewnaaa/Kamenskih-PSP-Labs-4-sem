...
2. [1. Теоретическая часть: Promise, async/await, fetch](#1-теоретическая-часть-promise-asyncawait-fetch)
   - 2.1 [1.1 Promise](#11-promise)
   - 2.2 [1.2 async/await](#12-asyncawait)
   - 2.3 [1.3 Fetch API](#13-fetch-api)
3. [2. Реализация в проекте](#2-реализация-в-проекте)
   - 3.1 [Модуль ajax.js (замена XHR на fetch)](#модуль-ajaxjs-замена-xhr-на-fetch)
   - 3.2 [2.2 Использование в страницах](#22-использование-в-страницах)
4. [3. Сборка клиентской части с помощью Vite](#3-сборка-клиентской-части-с-помощью-vite)
   - 4.1 [3.1 Установка и настройка](#31-установка-и-настройка)
   - 4.2 [3.2 Режимы работы](#32-режимы-работы)
5. [4. Интеграция с бэкендом: раздача статики](#4-интеграция-с-бэкендом-раздача-статики)
   - 5.1 [4.1 Настройка Express.js](#41-настройка-expressjs)
   - 5.2 [4.2 Запуск приложения](#42-запуск-приложения)
6. [5. Выводы](#5-выводы)

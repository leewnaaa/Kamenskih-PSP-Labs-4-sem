...
1. Цель работы
2. Инструменты
3. Создание слоя для работы с API
   - 3.1. Модуль `stockUrls.js`
   - 3.2. Модуль ajax.js (обёртка над XHR)
4. Реализация API на главной странице (список счетов)
   - 4.1. Загрузка списка при загрузке страницы
   - 4.2. Фильтрация по названию
   - 4.3. Создание нового счёта (POST)
5. Реализация API на странице деталей счёта
   - 5.1. Загрузка одного счёта по ID
   - 5.2. Навигация назад
